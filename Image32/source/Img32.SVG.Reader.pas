...
{$ENDIF}
  ...
  protected
    ...
    procedure DrawFilled(img: TImage32; const paths: TPathsD;
      drawDat: TDrawData);
    procedure DrawStroke(img: TImage32; const paths: TPathsD;
  ...
  public
    ...
    property KeepAspectRatio: Boolean read fUsePropScale write fUsePropScale;
...
type
  ...
  private
    fSvgPaths: TSvgPath;
    procedure Flatten(index: Integer; scalePending: double; out Path: TPathD;
      out isClosed: Boolean);
  protected
  ...
  protected
    ...
    pattBoxWH: TRectWH;
    function PrepareRenderer(renderer: TImageRenderer; drawDat: TDrawData)
      : Boolean; virtual;
  public
    constructor Create(parent: TBaseElement; svgEl: TSvgXmlEl); override;
    destructor Destroy; override;
  ...
  protected
    ...
    procedure Apply(img: TImage32; const filterBounds: TRect;
...
const
  ...
  emptyDrawInfo: TDrawData = (currentColor: clInvalid; fillColor: clInvalid;
    fillOpacity: InvalidD; fillRule: frNegative; fillEl: '';
    strokeColor: clInvalid; strokeOpacity: InvalidD;
    strokeWidth: (rawVal: InvalidD; unitType: utNumber); strokeCap: esClosed;
    strokeJoin: jsMiter; strokeMitLim: 0.0; strokeEl: ''; dashArray: nil;
    dashOffset: 0; fontInfo: (family: tfUnknown; familyNames: nil; size: 0;
    spacing: 0.0; spacesInText: sitUndefined; textLength: 0;
    italic: sfsUndefined; weight: - 1; align: staUndefined;
    decoration: fdUndefined; baseShift: (rawVal: InvalidD; unitType: utNumber));
    markerStart: ''; markerMiddle: ''; markerEnd: ''; filterElRef: '';
    maskElRef: ''; clipElRef: ''; matrix: ((1, 0, 0), (0, 1, 0), (0, 0, 1));
    visible: true; useEl: nil; bounds: (Left: 0; Top: 0; Right: 0; Bottom: 0));
...
begin
  with thisElement.fDrawData do begin
...
begin
  with thisElement.fDrawData do begin
    if fontInfo.family <> tfUnknown then begin
    ...
    if (thisElement is TTextElement) or (fontInfo.decoration <> fdUndefined)
    then
...
begin
  with drawDat do
    Result:=(fillOpacity <> 0) and ((fillColor <> clNone32) or (fillEl <> ''));
end;
// ------------------------------------------------------------------------------
...
begin
  if (opacity < 0) or (opacity >= 1.0) then
    Result:=color or $FF000000
  else if opacity = 0 then
    Result:=clNone32
  else
    Result:=(color and $FFFFFF) + Round(opacity * 255) shl 24;
end;
// ------------------------------------------------------------------------------
...
function UTF8StringToFloatEx(const ansiValue: UTF8String; var value: double;
  out measureUnit: TUnitType): Boolean;
var
  C: PUTF8Char;
begin
  C:=PUTF8Char(ansiValue);
...
var
  opacity: double;
begin
  if color = clNone32 then begin
    color:=clAlphaSet;
    Exit;
  end;

  if color = clInvalid then
    color:=clNone32;
  if not UTF8StringToFloat(ansiValue, opacity) then
    Exit;
  with TARGB(color) do
...
function MatrixApply(const paths: TPathsD; const matrix: TMatrixD)
  : TPathsD; overload;
var
...
begin
  if not Assigned(paths) then
    ...
    for i:=0 to len - 1 do begin
      ...
      pp:=@paths[i][0];
      rr:=@Result[i][0];
      for j:=0 to High(paths[i]) do begin
...
function FixSpaces(const Text: UnicodeString; trimLeadingSpace: Boolean)
  : UnicodeString;
var
  i, j, len: Integer;
begin
  ...
  if trimLeadingSpace then begin
    i:=1;
    while (i <= len) and (Text[i] <= #32) do
      inc(i);
    if i > len then begin
      Result:='';
      Exit;
...
begin
  ...
  for i:=1 to Length(Text) do
    if (Text[i] > #32) and (Text[i] <> #160) then
      Exit;
  Result:=true;
end;
// ------------------------------------------------------------------------------
...
begin
  ...
  while (Result <> -1) and ((FItems[Result].Hash <> Hash) or
    not IsSameUTF8String(FItems[Result].Name, Name)) do
    Result:=FItems[Result].Next;
end;
// ------------------------------------------------------------------------------

procedure TSvgIdNameHashMap.AddOrIgnore(const idName: UTF8String;
  Element: TBaseElement);
var
  index: Integer;
  Hash: Cardinal;
...
begin
  ...
  C:=PUTF8Char(s);
  if not Match(C, 'data:image/') then
    Exit;

  if Match(@C[11], 'jpg;base64,') then
    offset:=22
  else if Match(@C[11], 'jpeg;base64,') then
    offset:=23
  else if Match(@C[11], 'png;base64,') then
    offset:=22
  else
    Exit;
  ...
  try
    if not Base64Decode(@C[offset], len - offset, ms) then
      Exit;
    img:=TImage32.Create;
    if not img.LoadFromStream(ms) then begin
      FreeAndNil(img);
      Exit;
    end;
  finally
...
begin
  ...
  if (fRefEl <> '') and not Assigned(fImage) then begin
  ...
  if fImage <> nil then begin
    if elRectWH.IsValid then begin
...
begin
  ...
  if Assigned(clipEl) then begin
    drawDat.clipElRef:='';
    with clipEl do begin
      GetPaths(drawDat);
    ...
    if offsetY < 0 then
      offsetY:=0;
    if (offsetX > 0) or (offsetY > 0) then begin
    ...
    tmpImg:=TImage32.Create(Min(image.Width, clipRec.Right),
      Min(image.Height, clipRec.Bottom));
    try
      ...
      EraseOutsidePaths(tmpImg, clipPaths, fr, clipRec,
  ...
  else if Assigned(maskEl) then begin
    drawDat.maskElRef:='';
    with maskEl do begin
      GetPaths(drawDat);
      clipRec:=maskRec;
    end;
  ...
  for i:=0 to fChilds.Count - 1 do
    if TBaseElement(fChilds[i]) is TShapeElement then
      with TShapeElement(fChilds[i]) do
        if fDrawData.visible then begin
          Draw(image, drawDat);
...
var
  ...
  dx, dy: double;
begin
  if pathsLoaded or (fRefEl = '') then
    Exit;

  el:=FindRefElement(fRefEl);
  if not Assigned(el) or not(el is TShapeElement) then
    Exit;
  pathsLoaded:=true;

  with TShapeElement(el) do begin
    GetPaths(drawDat);
  ...
  if elRectWH.Left.IsValid then
    dx:=elRectWH.Left.rawVal
  else
    dx:=0;
  if elRectWH.Top.IsValid then
    dy:=elRectWH.Top.rawVal
  else
    dy:=0;
...
begin
  ...
  if not ValidateNonRecursion(drawDat.useEl) then
    Exit;
  ...
  if el is TSymbolElement then begin
    with TSymbolElement(el) do begin
      if not viewboxWH.IsEmpty then begin
        // scale the symbol according to its width and height attributes
        if elRectWH.Width.IsValid and elRectWH.Height.IsValid then begin
          scale2.X:=elRectWH.Width.rawVal / viewboxWH.Width;
          scale2.Y:=elRectWH.Height.rawVal / viewboxWH.Height;
          if scale2.Y < scale2.X then
            s:=scale2.Y
          else
            s:=scale2.X;
        ...
        if Self.elRectWH.Width.IsValid and Self.elRectWH.Height.IsValid then
        begin
          with viewboxWH do begin
...
begin
  maskRec:=NullRect;
  for i:=0 to fChilds.Count - 1 do
    if TBaseElement(fChilds[i]) is TShapeElement then begin
      el:=TShapeElement(fChilds[i]);
      el.GetPaths(drawDat);
      maskRec:=Img32.Vector.UnionRect(maskRec,
        Img32.Vector.GetBounds(el.drawPathsC));
...
var
  tmpImg: TImage32;
begin
  tmpImg:=TImage32.Create(Min(img.Width, maskRec.Right),
    Min(img.Height, maskRec.Bottom));
  try
    DrawChildren(tmpImg, drawDat);
...
var
  i, len: Integer;
begin
  if not Assigned(other) or not(other is TGradientElement) then
    Exit;
  inherited;

  with TGradientElement(other) do begin
    if units = 0 then
      units:=Self.units;

    if Length(stops) = 0 then begin
...
function TGradientElement.PrepareRenderer(renderer: TCustomGradientRenderer;
...
procedure TGradientElement.AddColorStopsToRenderer
  (renderer: TCustomGradientRenderer);
var
  i, hiStops: Integer;
begin
  hiStops:=High(stops);
  if (hiStops = 0) or (renderer = nil) then
    Exit;
...
begin
  ...
  if other is TRadGradElement then
    with TRadGradElement(other) do begin
      if not radius.IsValid then
        radius:=Self.radius;
...
begin
  ...
  if units = hUserSpaceOnUse then
    rec2:=fSvgReader.userSpaceBounds
  else
    rec2:=drawDat.bounds;

  if radius.IsValid then begin
    if radius.X.HasFontUnits then
      r:=radius.GetPoint(drawDat.fontInfo.size, GetRelFracLimit)
    else
  ...
  if C.IsValid then begin
    if C.X.HasFontUnits then
      cp:=C.GetPoint(drawDat.fontInfo.size, GetRelFracLimit)
    else
  ...
  if F.IsValid then begin
    if F.X.HasFontUnits then
      fp:=F.GetPoint(drawDat.fontInfo.size, GetRelFracLimit)
    else
  ...
  with renderer as TSvgRadialGradientRenderer do begin
    SetParameters(Rect(rec3), Point(fp), stops[0].color, stops[hiStops].color,
      spreadMethod);
...
begin
  ...
  if other is TLinGradElement then
    with TLinGradElement(other) do begin
      if not startPt.IsValid then
        startPt:=Self.startPt;
      if not endPt.IsValid then
        endPt:=Self.endPt;
    end;
end;
// ------------------------------------------------------------------------------

function TLinGradElement.PrepareRenderer(renderer: TCustomGradientRenderer;
  drawDat: TDrawData): Boolean;
var
  pt1, pt2: TPointD;
  hiStops: Integer;
...
begin
  ...
  with TLinearGradientRenderer(renderer) do begin
    if startPt.X.HasFontUnits then
      pt1:=startPt.GetPoint(drawDat.fontInfo.size, GetRelFracLimit)
    else
      pt1:=startPt.GetPoint(rec2, GetRelFracLimit);

    if (startPt.X.unitType <> utPixel) or (units <> hUserSpaceOnUse) then
      pt1.X:=pt1.X + rec2.Left;

    if (startPt.Y.unitType <> utPixel) or (units <> hUserSpaceOnUse) then
    ...
    if (units <> hUserSpaceOnUse) and ((pt2.X <> pt1.X) or (pt2.Y <> pt1.Y))
    then begin
      // skew the gradient
    end;

    SetParameters(pt1, pt2, stops[0].color, stops[hiStops].color, spreadMethod);
    AddColorStopsToRenderer(renderer);
  end;
end;
  ...
  if elRectWH.IsValid then begin
    ...
    for i:=0 to ChildCount - 1 do begin
      if Child[i] is TFeGaussElement then begin
        ...
        with TFeOffsetElement(Child[i]) do begin
          pt:=offset.GetPoint(bounds, 1);
    ...
    if (delta.cx = InvalidD) or (delta.cy = InvalidD) then
      Exit;

    // limit the filter margin to 20% if only blurring
    if not hasOffset then
      with delta, bounds do begin
        if cx > Width * 0.2 then
          cx:=Width * 0.2;
...
function TFilterElement.GetNamedImage(const Name: UTF8String; isIn: Boolean)
  : TImage32;
begin
  ...
  case GetHash(name) of
    ...
    hBackgroundAlpha: begin
        Result.Copy(fSvgReader.BackgndImage, fFilterBounds, Result.bounds);
        Result.SetRGB(clNone32, Result.bounds);
      end;
    hSourceGraphic: Result.Copy(fSrcImg, fFilterBounds, Result.bounds);
    hSourceAlpha: begin
...
procedure TFilterElement.Apply(img: TImage32; const filterBounds: TRect;
  const matrix: TMatrixD);
var
  i: Integer;
begin
...
begin
  ...
  if (res <> '') then
    dstImg:=pfe.GetNamedImage(res, false)
  else
...
begin
  if not GetSrcAndDst then
    Exit;
  pfe:=ParentFilterEl;
  if (in2 = '') then
    Exit;
  if dstImg = srcImg then
    dstImg2:=pfe.AddNamedImage(tmpFilterImg)
  else
    dstImg2:=dstImg;
...
begin
  ...
  res.r:=ClampByte((ks[0] * c1.r * c2.r + ks[1] * c1.r * 255 + ks[2] * c2.r *
    255 + ks[3] * 65025) shr 16);
  res.G:=ClampByte((ks[0] * c1.G * c2.G + ks[1] * c1.G * 255 + ks[2] * c2.G *
    255 + ks[3] * 65025) shr 16);
  res.B:=ClampByte((ks[0] * c1.B * c2.B + ks[1] * c1.B * 255 + ks[2] * c2.B *
    255 + ks[3] * 65025) shr 16);
end;
// ------------------------------------------------------------------------------
...
begin
  ...
  if (w2 <> w) or (w3 <> w) or (h2 <> h) or (h3 <> h) or (ks[0] = InvalidD) or
    (ks[1] = InvalidD) or (ks[2] = InvalidD) or (ks[3] = InvalidD) then
    Exit;
...
begin
  ...
  case compositeOp of
    ...
    coArithmetic: begin
        ArithmeticBlend(srcImg, srcImg2, dstImg2, srcRec, srcRec2,
          dstRec2, fourKs);
      end;
  else // coOver
    begin
...
var
  ...
  p1, p2: PColor32;
begin
  if not GetSrcAndDst or not Assigned(values) then
    Exit;
  for i:=0 to 19 do
  ...
  for i:=srcRec.Top to srcRec.Bottom - 1 do begin
    for j:=srcRec.Left to srcRec.Right - 1 do begin
...
begin
  if not GetSrcAndDst or (ChildCount = 0) then
    Exit;
  for i:=0 to 3 do
    childFuncs[i]:=nil;
  for i:=0 to ChildCount - 1 do begin
  ...
  for k:=0 to 3 do
    with childFuncs[k] do begin
      if not Assigned(childFuncs[k]) then
        Continue;
      case funcType of
        ftDiscrete: begin
            if Length(tableValues) = 0 then
              Continue;
            SetLength(bytes, 256);
  ...
  for i:=srcRec.Top to srcRec.Bottom - 1 do begin
    for j:=srcRec.Left to srcRec.Right - 1 do begin
      p2.color:=p1^;
      if Assigned(childFuncs[0]) then
        p2.B:=childFuncs[0].bytes[p2.B];
      if Assigned(childFuncs[1]) then
        p2.G:=childFuncs[1].bytes[p2.G];
      if Assigned(childFuncs[2]) then
        p2.r:=childFuncs[2].bytes[p2.r];
      if Assigned(childFuncs[3]) then
...
begin
  ...
  if elRectWH.IsValid then
    rec:=Rect(elRectWH.GetRectD(RectD(srcRec), GetRelFracLimit))
  else
    rec:=dstRec;
...
begin
  if (stdDev = InvalidD) or not GetSrcAndDst then
    Exit;
...
begin
  ...
  for i:=0 to fChilds.Count - 1 do
    if TBaseElement(fChilds[i]) is TFeMergeNodeElement then
      with TFeMergeNodeElement(fChilds[i]) do begin
        if not GetSrcAndDst then
          Continue;
...
begin
  ...
  for i:=0 to fChilds.Count - 1 do
    if TBaseElement(fChilds[i]) is TShapeElement then
      with TShapeElement(fChilds[i]) do begin
        GetPaths(drawDat);
...
begin
  ...
  if not(filled or stroked) or not hasPaths then begin
    inherited;
    Exit;
  end;
  tmpRec:=GetBounds;
  ...
  useTmpImage:=Assigned(clipPathEl) or Assigned(filterEl) or Assigned(maskEl);

  if useTmpImage then begin
    img:=fSvgReader.TempImage;

    // get special effects bounds
    if Assigned(clipPathEl) then begin
      drawDat.clipElRef:='';
      di:=drawDat;
      with TClipPathElement(clipPathEl) do begin
        GetPaths(di);
      ...
      drawDat.maskElRef:='';
      with TMaskElement(maskEl) do begin
        GetPaths(drawDat);
      ...
      if clipRec.IsEmpty and (drawDat.fontInfo.textLength > 0) and
        (Self is TTextPathElement) then begin
      ...
      if stroked and drawDat.strokeWidth.IsValid then begin
        with drawDat.strokeWidth do
          if HasFontUnits then
            d:=GetValue(drawDat.fontInfo.size, GetRelFracLimit)
          else
            d:=GetValueXY(clipRec, GetRelFracLimit);
        Img32.Vector.InflateRect(clipRec, d * 0.5, d * 0.5);
      end;
      if Assigned(filterEl) then begin
        drawDat.filterElRef:='';
        with TFilterElement(filterEl) do begin
  ...
  if Assigned(drawPathsC) or Assigned(drawPathsO) then begin
...
begin
  ...
  if not Assigned(markerPaths) then
    Exit;
  ...
  with fDrawData.strokeWidth do
    if not IsValid then
      ...
      sw:=GetValueXY(drawDat.bounds, GetRelFracLimit);

  MatrixExtractScale(drawDat.matrix, scale);
  MatrixScale(di.matrix, sw * scale);
  if (fDrawData.markerStart <> '') then begin
    markerEl:=FindRefElement(fDrawData.markerStart);
    if Assigned(markerEl) and (markerEl is TMarkerElement) then
      with TMarkerElement(markerEl) do begin
        for i:=0 to High(markerPaths) do begin
          if Length(markerPaths[i]) < 2 then
            Continue;
          pt1:=markerPaths[i][0];
          pt2:=markerPaths[i][1];
  ...
  if (fDrawData.markerMiddle <> '') then begin
  ...
  if (fDrawData.markerEnd <> '') then begin
...
procedure TShapeElement.DrawFilled(img: TImage32; const paths: TPathsD;
  drawDat: TDrawData);
var
  refEl: TBaseElement;
  rec: TRect;
...
begin
  ...
  if (drawDat.fillEl <> '') then begin
    refEl:=FindRefElement(drawDat.fillEl);
    if Assigned(refEl) and (refEl is TFillElement) then begin
      if refEl is TRadGradElement then begin
        with TRadGradElement(refEl) do begin
          ...
          if PrepareRenderer(fSvgReader.RadGradRenderer, drawDat) then
            DrawPolygon(img, paths, drawDat.fillRule,
              fSvgReader.RadGradRenderer);
        end;
      end
      else if refEl is TLinGradElement then begin
        with TLinGradElement(refEl) do begin
          fSvgReader.LinGradRenderer.opacity:=opacity;
          if PrepareRenderer(fSvgReader.LinGradRenderer, drawDat) then
            DrawPolygon(img, paths, drawDat.fillRule,
              fSvgReader.LinGradRenderer);
        end;
      end
      else if refEl is TPatternElement then begin
        with TPatternElement(refEl) do
          if PrepareRenderer(ImgRenderer, drawDat) then begin
  ...
  else if drawDat.fillColor = clInvalid then begin
    DrawPolygon(img, paths, drawDat.fillRule, MergeColorAndOpacity(clBlack32,
      drawDat.fillOpacity), fSvgReader.fCustomRendererCache);
  end
  else
    with drawDat do begin
      DrawPolygon(img, paths, fillRule, MergeColorAndOpacity(fillColor,
        fillOpacity), fSvgReader.fCustomRendererCache);
    end;
end;
// ------------------------------------------------------------------------------

procedure TShapeElement.DrawStroke(img: TImage32; const paths: TPathsD;
  drawDat: TDrawData; isClosed: Boolean);
var
  i: Integer;
...
begin
  ...
  with drawDat.strokeWidth do begin
    if not IsValid then
      sw:=1
    else if HasFontUnits then
      sw:=GetValue(drawDat.fontInfo.size, GetRelFracLimit)
  ...
  if Length(drawDat.dashArray) > 0 then
    dashArray:=ScaleDashArray(drawDat.dashArray, scale)
  else
    dashArray:=nil;
  ...
  if isClosed then begin
    if Assigned(dashArray) then begin
      ...
      strokePaths:=RoughOutline(strokePaths, sw, joinStyle, endStyle,
        miterLim, scale);
    end
    else begin
      endStyle:=esClosed;
      strokePaths:=RoughOutline(paths, sw, joinStyle, endStyle,
        miterLim, scale);
    end;
  end
  else begin
    ...
    if Assigned(dashArray) then begin
      ...
      DrawDashedLine(img, strokePaths, dashArray, @dashOffset, sw * scale,
        strokeClr, endStyle, jsAuto, fSvgReader.fCustomRendererCache);
      Exit;
    end;
    strokePaths:=RoughOutline(paths, sw, joinStyle, endStyle, miterLim, scale);
  end;
  strokePaths:=MatrixApply(strokePaths, drawDat.matrix);

  if (drawDat.strokeEl <> '') then begin
    refEl:=FindRefElement(drawDat.strokeEl);
    if not Assigned(refEl) then
      Exit;
    ...
    if refEl is TRadGradElement then begin
      ...
      with TPatternElement(refEl) do begin
...
begin
  ...
  for i:=0 to fSvgPaths.Count - 1 do begin
...
begin
  if pathsLoaded or not Assigned(Path) then
    Exit;
  pathsLoaded:=true;
  if (fXmlEl.Hash = hPolygon) then begin
...
begin
  ...
  endC:=C + Length(value);
  SetLength(Path, currCap);
  while IsNumPending(C, endC, true) and ParseNextNum(C, endC, true, pt.X) and
...
var
  ...
  r: double;
begin
  if pathsLoaded or not radius.IsValid then
    Exit;
  pathsLoaded:=true;
...
begin
  ...
  if elRectWH.Width.HasFontUnits then
    bounds:=elRectWH.GetRectD(drawDat.fontInfo.size, GetRelFracLimit)
  else
  ...
  if (radXY.X > 0) or (radXY.Y > 0) then begin
    if (radXY.X <= 0) then
      radXY.X:=radXY.Y
    else if (radXY.Y <= 0) then
      radXY.Y:=radXY.X;
...
begin
  ...
  if not Assigned(fSvgReader.fFontCache) then
    Exit;
  if drawDat.fontInfo.size = 0 then
    drawDat.fontInfo.size:=16;
...
var
  el: TBaseElement;
begin
  if not Assigned(textEl) then begin
    el:=fParent;
...
begin
  ...
  GetTextEl;
  if not Assigned(textEl) or (textEl.currentPt.X = InvalidD) or
    (textEl.currentPt.Y = InvalidD) then
    Exit;
  ...
  if elRectWH.Left.IsValid then begin
  ...
  if offset.X.IsValid then begin
  ...
  if not fXmlEl.selfClosed then begin
    textEl.currSpanEl:=Self;
    angles:=nil;
  end
  else if (fParent is TTSpanElement) then begin
    if Assigned(TTSpanElement(fParent).angle) then
      angles:=TTSpanElement(fParent).angle
    else
      angles:=textEl.angle;
  end
  else begin
  ...
  chunkDx:=0;
  if (Length(fXmlEl.Text) > 0) and (fontSize > 1) then begin
    ...
    s:=FixSpaces(s, textEl.lastChrWasSpc or ((fParent = textEl) and
      (Self = textEl.Child[0])));

    if IsBlankText(s) then begin
      drawPathsC:=nil;
      // don't allow duplicate spaces or a space at the beginning of text
      if textEl.lastChrWasSpc or (Self = textEl.Child[0]) then
        Exit;
      tmpX:=fSvgReader.fFontCache.GetSpaceWidth;
      textEl.lastChrWasSpc:=true;
    end
    else if Assigned(angles) then begin
      drawPathsC:=nil;
      tmpPaths:=nil;
      tmpX:=0;
      ...
      if len > Length(angles) then begin
        j:=High(angles);
      ...
      for i:=0 to len - 1 do begin
        ...
        if Assigned(glyphInfo.paths) then begin
    ...
    with textEl.currentPt do begin
      startX:=X;
      X:=X + chunkDx;
    end;
...
begin
  ...
  filled:=IsFilled(drawDat);
  stroked:=IsStroked(drawDat);
  if Assigned(drawPathsC) and Assigned(textEl) then begin
    ...
    if (fParent is TTSpanElement) and fParent.elRectWH.Left.IsValid then begin
    ...
    else if textEl.textDx <> 0 then begin
      case drawDat.fontInfo.align of
        staCenter:
            drawPathsC:=TranslatePath(drawPathsC, -textEl.textDx * 0.5, 0);
...
var
  ...
  isClosed: Boolean;
begin
  if not(el is TTSpanElement) then
    Exit;
  spanEl:=TTSpanElement(el);
  if Assigned(spanEl.drawPathsC) then
    Exit;
  ...
  if dd.fontInfo.spacesInText <> sitPreserve then
    unicodeText:=TrimMultiSpacesUnicode(unicodeText)
  else
  ...
  if (len < 2) then
    spacing:=0
  else if (dd.fontInfo.align = staJustify) and (pathEl.fSvgPaths.Count = 1) then
    with TPathElement(pathEl) do begin
  ...
  else if (dd.fontInfo.textLength > 0) then begin
    textWidth:=fSvgReader.fFontCache.GetTextWidth(unicodeText);
    spacing:=(dd.fontInfo.textLength / scale) - textWidth;
    spacing:=spacing / (len - 1);
  end;

  with pathEl do begin
    ...
    for i:=0 to fSvgPaths.Count - 1 do begin
      ...
      tmpPaths:=GetTextOutlineOnPath(unicodeText, tmpPath,
        fSvgReader.fFontCache, taLeft, textEl.currentPt.X, textEl.currentPt.Y,
        spacing, charsThatFit, outX);
      AppendPath(spanEl.drawPathsC, tmpPaths);
      textEl.currentPt.X:=outX;
      if charsThatFit = Length(unicodeText) then
        break;
...
var
  ...
  el: TBaseElement;
begin
  if pathsLoaded or not Assigned(fSvgReader.fFontCache) then
    Exit;
  pathsLoaded:=true;
  GetTextEl;
  ...
  if not(el is TPathElement) then
    Exit;
  ...
  if offset.X.IsValid then
    textEl.currentPt.X:=
      Max(0, textEl.currentPt.X + Round(offset.X.GetValue(dd.bounds.Width,
      1) / scale));
  if offset.Y.IsValid then
    textEl.currentPt.Y:=textEl.currentPt.Y + Round(offset.Y.rawVal / scale);
...
{$ENDIF}
    Result:=textEl.fDrawData.bounds
  else
...
begin
  if pathsLoaded then
    Exit;
  Text:=fXmlEl.Text;
  if not elRectWH.Width.IsValid or not elRectWH.Height.IsValid or (Text = '')
  then
    Exit;
  pathsLoaded:=true;
  ...
  with TChunkedText.Create(s, fSvgReader.fFontCache) do
    try
      ...
      drawPathsC:=GetTextGlyphs(Rect(textRec), taLeft, tvaTop, NullPoint,
        lnHeight * 0.8);
    finally
      Free;
    end;
...
begin
  ...
  if elRectWH.Width.IsValid and elRectWH.Height.IsValid and not markerBoxWH.IsEmpty
  then begin
  ...
  if refPt.X.IsValid and refPt.Y.IsValid then begin
    l:=refPt.X.rawVal;
    t:=refPt.Y.rawVal;
...
begin
  ...
  if units = hUserSpaceOnUse then
    rec:=fSvgReader.userSpaceBounds
  else
    rec:=drawDat.bounds;
  ...
  if elRectWH.Width.IsValid and elRectWH.Height.IsValid then begin
    ...
    if recWH.IsEmpty then
      Exit;

    // also scale if necessary
    if not pattBoxWH.IsEmpty then begin
      sx:=recWH.Width / pattBoxWH.Width;
      sy:=recWH.Height / pattBoxWH.Height;
    end;

  end
  else if not pattBoxWH.IsEmpty then begin
  ...
  renderer.image.SetSize(Round(recWH.Width * scale.X),
  ...
  for i:=0 to fChilds.Count - 1 do
    if TBaseElement(fChilds[i]) is TShapeElement then
      ...
      with TImageElement(fChilds[i]) do begin
        drawDat:=fDrawData;
        drawDat.matrix:=mat;
        drawDat.bounds:=rec;
        Draw(renderer.image, drawDat);
...
begin
  dd:=drawDat;
  if (fSvgReader.RootElement <> Self) then begin
    if (elRectWH.Left.rawVal <> InvalidD) or (elRectWH.Top.rawVal <> InvalidD)
    then begin
      MatrixExtractScale(dd.matrix, sx, sy);
      MatrixTranslate(dd.matrix, elRectWH.Left.rawVal * sx,
...
var
  i: Integer;
begin
  if Match(Self.fId, idName) then begin
    Result:=Self;
    Exit;
  end;
...
begin
  if (value = '') or not(aOwnerEl is TFeComponentTransferChild) then
    Exit;
  with TFeComponentTransferChild(aOwnerEl) do begin
    len:=0;
    tableValues:=nil;
    C:=PUTF8Char(value);
    endC:=C + Length(value);
    while ParseNextNum(C, endC, true, val) do begin
...
begin
  case aOwnerEl.fXmlEl.Hash of
    hFeImage: TFeImageElement(aOwnerEl).refEl:=ExtractRef(value);
    hImage: TImageElement(aOwnerEl).fRefEl:=ExtractRef(value);
    hUse: TUseElement(aOwnerEl).fRefEl:=ExtractRef(value);
    hTextPath: TTextPathElement(aOwnerEl).pathName:=ExtractRef(value);
  else
...
begin
  case aOwnerEl.fXmlEl.Hash of
    ...
    hfeFlood: UTF8StringToColor32(value, TFeFloodElement(aOwnerEl).floodColor);
  else begin
...
begin
  case aOwnerEl.fXmlEl.Hash of
    ...
    hfeFlood: UTF8StringToOpacity(value, TFeFloodElement(aOwnerEl).floodColor);
  else begin
      UTF8StringToFloat(value, opacity);
      with aOwnerEl.fDrawData do begin
...
var
  ...
  C, endC: PUTF8Char;
begin
  with aOwnerEl.fDrawData.fontInfo do begin
    family:=tfUnknown;
    ...
    C:=PUTF8Char(value);
    endC:=C + Length(value);
    while ParseNextWordExHash(C, endC, Hash) do begin
      case Hash of
...
begin
  C:=PUTF8Char(value);
  endC:=C + Length(value);
  if not ParseNextNum(C, endC, false, num) then
    Exit;
...
begin
  if (aOwnerEl is TRadGradElement) then
    with TRadGradElement(aOwnerEl) do begin
...
procedure TextDecoration_Attrib(aOwnerEl: TBaseElement;
...
begin
  if not(aOwnerEl is TShapeElement) then
    Exit;
...
begin
  if (aOwnerEl is TGradStopElement) then
    with TGradStopElement(aOwnerEl) do begin
      val.Init;
...
var
  opacity: double;
begin
  if not UTF8StringToFloat(value, opacity) then
    Exit;
...
var
  val: double;
begin
  if (aOwnerEl is TTextPathElement) and UTF8StringToFloat(value, val) then
...
procedure StrokeLineJoin_Attrib(aOwnerEl: TBaseElement;
  const value: UTF8String);
var
  Hash: Cardinal;
  C, endC: PUTF8Char;
...
procedure StrokeMiterLimit_Attrib(aOwnerEl: TBaseElement;
  const value: UTF8String);
begin
...
begin
  if LowerCaseTable[value[1]] = 'e' then
    aOwnerEl.fDrawData.fillRule:=frEvenOdd
  else
...
begin
  ...
  endC:=C + Length(value);
  cnt:=0;
  while ParseNextNum(C, endC, true, angle[cnt]) do
    inc(cnt);
  SetLength(angle, cnt);
...
begin
  if aOwnerEl is TFeColorMatrixElement then
    with TFeColorMatrixElement(aOwnerEl) do begin
      SetLength(values, 20);
      C:=PUTF8Char(value);
      endC:=C + Length(value);
      cnt:=0;
...
procedure GradientTransform_Attrib(aOwnerEl: TBaseElement;
  const value: UTF8String);
var
  mat: TMatrixD;
begin
  if not(aOwnerEl is TGradientElement) then
    Exit;
  ...
  begin
    ...
    with Result do
      if not ParseNextNum(C, endC, false, Left) or
        not ParseNextNum(C, endC, true, Top) or not ParseNextNum(C, endC, true,
        Width) or not ParseNextNum(C, endC, true, Height) then
...
begin
  case aOwnerEl.fXmlEl.Hash of
    ...
    if aOwnerEl is TPatternElement then
...
begin
  UTF8StringToFloatEx(value, val, mu);
  case aOwnerEl.fXmlEl.Hash of
    hfeDropShadow: TFeDropShadowElement(aOwnerEl).offset.X.SetValue(val, mu);
    hfeOffset: TFeOffsetElement(aOwnerEl).offset.X.SetValue(val, mu);
    hText: TTextElement(aOwnerEl).offset.X.SetValue(val, mu);
    hTSpan: TTSpanElement(aOwnerEl).offset.X.SetValue(val, mu);
    hTextPath: TTextPathElement(aOwnerEl).offset.X.SetValue(val, mu);
  end;
end;
// ------------------------------------------------------------------------------
...
begin
  UTF8StringToFloatEx(value, val, mu);
  case aOwnerEl.fXmlEl.Hash of
    hfeDropShadow: TFeDropShadowElement(aOwnerEl).offset.Y.SetValue(val, mu);
    hfeOffset: TFeOffsetElement(aOwnerEl).offset.Y.SetValue(val, mu);
    hText: TTextElement(aOwnerEl).offset.Y.SetValue(val, mu);
    hTSpan: TTSpanElement(aOwnerEl).offset.Y.SetValue(val, mu);
    hTextPath: TTextPathElement(aOwnerEl).offset.Y.SetValue(val, mu);
  end;
end;
// ------------------------------------------------------------------------------
...
begin
  ...
  case aOwnerEl.fXmlEl.Hash of
    hRect:
      with TRectElement(aOwnerEl) do begin
        radius.X.SetValue(val, mu);
      end;
    hCircle:
      with TCircleElement(aOwnerEl) do begin
        radius.SetValue(val, mu);
      end;
    hEllipse:
      with TEllipseElement(aOwnerEl) do begin
        radius.X.SetValue(val, mu);
      end;
    hRadialgradient:
      with TRadGradElement(aOwnerEl) do begin
...
var
  se: double;
begin
  if not(aOwnerEl is TFeSpecLightElement) then
    Exit;
...
begin
  ...
  if (sd < 0) and (sd > 100) then
    Exit;
  case aOwnerEl.fXmlEl.Hash of
    hFeGaussianBlur: TFeGaussElement(aOwnerEl).stdDev:=sd;
    hfeDropShadow: TFeDropShadowElement(aOwnerEl).stdDev:=sd;
  end;
end;
// ------------------------------------------------------------------------------
...
begin
  UTF8StringToFloatEx(value, val, mu);
  case aOwnerEl.fXmlEl.Hash of
    hLine: TLineElement(aOwnerEl).Path[0].X:=val;
    hLineargradient:
      with TLinGradElement(aOwnerEl) do begin
  ...
  else aOwnerEl.elRectWH.Left.SetValue(val, mu);
  end;
end;
// ------------------------------------------------------------------------------
...
begin
  UTF8StringToFloatEx(value, val, mu);
  case aOwnerEl.fXmlEl.Hash of
    hLine: TLineElement(aOwnerEl).Path[1].X:=val;
    hLineargradient:
      with TLinGradElement(aOwnerEl) do begin
...
begin
  UTF8StringToFloatEx(value, val, mu);
  case aOwnerEl.fXmlEl.Hash of
    hLine: TLineElement(aOwnerEl).Path[0].Y:=val;
    hLineargradient:
      with TLinGradElement(aOwnerEl) do begin
  ...
  else aOwnerEl.elRectWH.Top.SetValue(val, mu);
  end;
end;
// ------------------------------------------------------------------------------
...
begin
  UTF8StringToFloatEx(value, val, mu);
  case aOwnerEl.fXmlEl.Hash of
    hLine: TLineElement(aOwnerEl).Path[1].Y:=val;
    hLineargradient:
      with TLinGradElement(aOwnerEl) do begin
...
begin
  if (val.rawVal = InvalidD) or (val.unitType = utUnknown) then
    Result:=tsUnknown
  else if val.unitType = utPercent then
    Result:=tsYes
  else if val.unitType <> utNumber then
    Result:=tsNo
  else if (Abs(val.rawVal) < 1) then
    Result:=tsYes
  else
    Result:=tsNo;
end;
...
begin
  ...
  if Assigned(fParent) and (fParent.fXmlEl.Hash <> hSvg) then begin
...
begin
  Result:=false;
  for i:=0 to fXmlEl.childs.Count - 1 do begin
    ...
    Self.fChilds.Add(el);
    el.LoadAttributes;
    if el.fXmlEl.childs.Count = 0 then
      Continue
    else if not el.LoadContent then
      Exit; // error
  end;
  Result:=true;
end;
...
var
  ...
  di: TDrawData;
begin
  if not Assigned(fRootElement) or not Assigned(img) then
    Exit;
  ...
  with fRootElement do begin
    ...
    if scaleToImage and not img.IsEmpty then begin
      ...
      if fUsePropScale then begin
        scale:=GetScaleForBestFit(viewboxWH.Width, viewboxWH.Height, img.Width,
          img.Height);
        scaleH:=scale;
      end
      else begin
      ...
      img.SetSize(Round(viewboxWH.Width * scale),
...
begin
  ...
  if fTempImage = nil then begin
    ...
    fTempImage:=TImage32.Create(Pixels, BackgndImage.Width,
      BackgndImage.Height);
...
begin
  Result:=false;
  if not Assigned(fSvgParser.svgTree) or (fSvgParser.svgTree.Hash <> hSvg) then
    Exit;
...
var
  dd: TDrawData;
begin
  if not Assigned(RootElement) or (color = clNone32) then
    Exit;
...
begin
  if Assigned(RootElement) then
    Result:=RootElement.FindChild(idName)
  else
    Result:=nil;
end;
// ------------------------------------------------------------------------------
...
begin
  ...
  if not Assigned(bestFontReader) then
    Exit;

  if Assigned(fFontCache) then
    fFontCache.FontReader:=bestFontReader
  else
